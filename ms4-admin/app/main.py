...
def verify_admin_token(token: str):
    """Vérifie si le token est valide et appartient à un admin"""
    # Mode développement
    if token == "fake-token-admin":
        return {"valid": True, "user": "admin1", "role": "admin"}
    elif token == "fake-token-enseignant":
        return {"valid": True, "user": "prof1", "role": "enseignant"}
    elif token == "fake-token-etudiant":
        return {"valid": True, "user": "etudiant1", "role": "etudiant"}
    else:
        return {"valid": False}
